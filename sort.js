...
function sortArray (arr) {
  ...
  for(var i in res) {
    ...
    while(arr.indexOf(value)>=0) {
      objArr[i].val = objArr[i].val+1;
      arr.splice(arr.indexOf(value),1);
    }
  }

  var res2 = objArr.sort(function(item,item2){
  ...
  if(node&&node.childNodes){
      ...
      for(var i in children) {
        ...
        if(n==1&&kid)
        {
          ...
          var next = getDescendants(node, n-1);
          if(next) {
            res = res.concat(next);
          }
        }
...
function unionSort(arr1, arr2) {
  ...
  return arr1.filter(function(){}).sort(function() {});
}

function unionSort2(arr1,arr2) {
  if(arr1&&arr2&&arr1.length>0&&arr2.length>0) {
    var index1=parseInt(arr1.length/2), index2= 0, len1 = arr1.length, len2 = arr2.length;
    while(index2<=len2) {
      if(arr1.indexOf(arr2[index2])) {
...
function getKV(url) {
  ...
  for(var i= 0,len=params.length;i<len;i++) {
    var list = params[i].split('=').map(function(item){
